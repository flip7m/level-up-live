...
import { getXPService, getLevelService, getPlaylistService, getEventService, getSessionService, getAssetService } from './utils/services.js'
...
export function createApp(): Express {
  ...
  // Asset endpoints (Animation System)
  try {
    const assetService = getAssetService()
    const assetController = new AssetController(assetService)
    app.use('/api/assets', assetController.getRouter())
    logger.info('AssetController routes registered successfully')
  } catch (err) {
    logger.error('Failed to initialize AssetController:', err)
  }
  ...
  // Stream endpoints (YouTube/Twitch streaming)
  const streamController = new StreamController()
  app.post('/api/stream/start', streamController.startStream)
  app.post('/api/stream/stop', streamController.stopStream)
  app.get('/api/stream/status', streamController.getStreamStatus)

  // SPA fallback - serve index.html for non-API routes (React Router)
  app.get('*', (req: Request, res: Response) => {
    if (!req.path.startsWith('/api') && !req.path.startsWith('/assets')) {
      res.sendFile('dist/client/index.html', { root: '.' }, (err) => {
        if (err) {
          logger.warn(`Failed to serve index.html for ${req.path}`)
  ...
  // 404 handler (fallback if no other route matches)
